...
func setNativeWindowIcon(window unsafe.Pointer, iconPath string) {
	...
	big, _, _ := loadImageWProc.Call(0, uintptr(unsafe.Pointer(path)), uintptr(imageIcon), 32, 32, uintptr(lrLoadFromFile))
	if big != 0 {
		_, _, _ = sendMessageWProc.Call(uintptr(window), uintptr(wmSetIcon), uintptr(iconBig), big)
	}
	small, _, _ := loadImageWProc.Call(0, uintptr(unsafe.Pointer(path)), uintptr(imageIcon), 16, 16, uintptr(lrLoadFromFile))
	if small != 0 {
