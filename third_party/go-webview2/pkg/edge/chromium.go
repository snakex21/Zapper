...
func (e *Chromium) CapturePreview(callback func([]byte, error)) error {
	...
	result, _, _ = e.webview.vtbl.CapturePreview.Call(
		...
		stream,
		uintptr(unsafe.Pointer(e.capturePreviewCompleted)),
	)
	if int32(result) < 0 {
		e.releaseCapturePreviewStream()
		e.capturePreviewCallback = nil
		return fmt.Errorf("CapturePreview failed with HRESULT 0x%08X", uint32(result))
	}
	return nil
}
...
func (e *Chromium) releaseCapturePreviewStream() {
	...
	stream := e.capturePreviewStream
	e.capturePreviewStream = 0
	unknown := (*struct{ vtbl *_IUnknownVtbl })(unsafe.Pointer(stream))
	_, _, _ = unknown.vtbl.Release.Call(stream)
}
