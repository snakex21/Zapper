...
The release version is defined by `appVersion` in `app/app.go`. Git tags must use the same version with a `v` prefix, for example `v10.4.1`.
...
For version 10.4.1 the expected files are:

```text
dist/Zapper-v10.4.1-Windows-x64.zip
dist/Zapper-v10.4.1-Windows-x64.zip.sha256
```
